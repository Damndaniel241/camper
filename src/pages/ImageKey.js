...
const ImageKey = () => {
    ...
    const handleImageUpload = async () => {
        ...
        try {
            const response = await axios.post('http://127.0.0.1:8000/api/one_time_image_keys/', formData, {
                headers: {
    ...
    //     try {
    //         const response = await axios.post('http://127.0.0.1:8000/api/verify_image/', formData, {
    //             headers: {
    //                 'Content-Type': 'multipart/form-data',
    //                 'Authorization': `Token ${localStorage.getItem('token')}`
    //             }
